...
class RegexFormatter(BaseFormatter):
	...
	def __init__(self):
		super().__init__()
		self.url_pattern = re.compile(r'https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+|www\.[\w-]+[\.[\w.-]+]+')
	...
	def _replace_romanian_date(self, match):
...
if __name__ == "__main__":
	...
	for formatter in formatters:
		texts = list(itertools.chain.from_iterable(
			[formatter.format(text) for text in batch] for batch in tqdm(dataloader, desc=f"Processing: {formatter.__class__.__name__}")
		))
